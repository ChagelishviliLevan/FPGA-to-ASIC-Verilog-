...
*calc* remains high while the calculation is running. After the final iteration, *calc* returns low and finish is asserted for one clock cycle. The calculated sine and cosine values are stored in *sin_out* and *cos_out*.
...
The sine and cosine outputs are *signed 16-bit fixed-point values with 15 fractional bits*. T
